...
function aesSuite(
  ...
  const encryptTiming = benchmark(() => {
    ...
    const cipher = crypto.createCipheriv(cipherName, key, iv) as crypto.CipherGCM;
  ...
  const decryptTiming = benchmark(() => {
    const decipher = crypto.createDecipheriv(cipherName, key, iv) as crypto.DecipherGCM;
